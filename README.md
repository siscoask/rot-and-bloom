...
- **Living compost bins** — each one visually darkens and settles as it decomposes, set into a textured soil bed rather than floating on empty space
- **Harvest** — a short reveal line, then the bin visibly transforms into a rooted plant in your yard (not a jump-cut)
- **Plant variety** — 7-day grudges grow a small sprout, 30-day a leafy plant, 90-day a small flowering shrub
- **Dig up early** — deleting a bin plays a crumble-and-scatter animation instead of just vanishing
...
- **Reduce motion** — turns off decay/reveal/crumble animations, also respects your system setting automatically
- **Custom install prompt** — an in-app banner offers to install, instead of relying on the hidden browser icon
- **Offline & update feedback** — a toast when you go offline, and a "Refresh" prompt when a new version is cached
- **Subtle haptics** — a light vibration on harvest and dig-up (devices that support it)
